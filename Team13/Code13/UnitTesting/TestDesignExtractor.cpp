...
public:
	...
	TEST_METHOD(validateAST_duplicateProc_DEExceptionThrown) {
		/* AST is equivalent to the SIMPLE program
		   procedure main {
				   print a; }
			procedure main {
		...
		*/
		...
		try {
			...
			Assert::AreEqual(DEException::DUPLICATE_PROC_NAMES.c_str(), ex.what());
		}
	}

	TEST_METHOD(validateAST_callNonExistentFunction_noContainerStmt_DEExceptionThrown) {
		/* AST is equivalent to the SIMPLE program
		   procedure main {
				   call proc1; }
			procedure proc {
		...
		*/
		...
		try {
			...
			Assert::AreEqual(DEException::CALL_NON_EXISTENT_PROC_NAME.c_str(), ex.what());
		}
	}

	TEST_METHOD(validateAST_callNonExistentFunction_insideOneContainerStmt_DEExceptionThrown) {
		/* AST is equivalent to the SIMPLE program
		   procedure main {
			while (x==y) {
				   call proc1; }
	...
	TEST_METHOD(validateAST_callNonExistentFunction_insideNestedContainerStmts_DEExceptionThrown) {
		/* AST is equivalent to the SIMPLE program
			procedure proc {
				   read a; }
			}
