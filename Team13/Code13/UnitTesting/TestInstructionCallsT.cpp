...
public:
	...
	//	Instruction* instruction = new RelationshipInstruction(PqlRelationshipType::CallsT, lhsRef, rhsRef);
	...
	//	uPredSucTable[procs[0]] = { procs[1] }; // 1 calls 2 (ignored)
	//	uPredSucTable[procs[1]] = { procs[2], procs[3] }; // 2 calls 3 and 4
	...
	//	}
	//	std::unordered_map<ProcIndex, std::unordered_set<ProcIndex>> uPredSucTable;
	//	uPredSucTable[procs[0]] = { procs[1] }; // 1 calls 2
	//	uPredSucTable[procs[1]] = { procs[2], procs[3] }; // 2 calls 3 and 4
	//	CallsT::populate(uPredSucTable);
	...
	//	uPredSucTable[procs[0]] = { procs[1] }; // 1 calls 2
	//	uPredSucTable[procs[1]] = { procs[2], procs[3] }; // 2 calls 3 and 4
	//	uPredSucTable[procs[2]] = { procs[3] }; // 3 calls 4
	//	CallsT::populate(uPredSucTable);
	...
	//	Assert::AreEqual(false, actualEntities.find("t") != actualEntities.end());
	...
	//	}
	//	std::unordered_map<ProcIndex, std::unordered_set<ProcIndex>> uPredSucTable;
	//	uPredSucTable[procs[0]] = { procs[1], procs[2] }; // 1 calls 2 and 3
	//	uPredSucTable[procs[1]] = { procs[2], procs[3] }; // 2 calls 3 and 4
	//	uPredSucTable[procs[2]] = { procs[3] }; // 3 calls 4
	//	CallsT::populate(uPredSucTable);
	...
	//	std::unordered_map<ProcIndex, std::unordered_set<ProcIndex>> uPredSucTable;
	//	for (int i = 0; i < 18; i++) {
	//		uPredSucTable[procs[i]] = { procs[i + 1] }; // i Calls i + 1, simulate 18 Calls relationships
	//	}
	//	CallsT::populate(uPredSucTable);
	...
	//		uPredSucTable[procs[i]] = { procs[i + 1] }; // i Calls i + 1, simulate 98 Calls relationships
	//	}
	//	CallsT::populate(uPredSucTable);
	...
	//	auto actualEntities = evTable.getEntities();
	//	Assert::AreEqual(false, actualEntities.find("_") != actualEntities.end());
	//	Assert::AreEqual(false, actualEntities.find("procedure87") != actualEntities.end());
